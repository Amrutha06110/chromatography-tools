...
_trapezoid: Callable[..., float] = getattr(np, "trapezoid", None) or np.trapz
...
class BaseChromatogram(ABC):
    ...
    Subclasses **must** implement :pyattr:`x_axis_label`,
    :pyattr:`y_axis_label`, and :pyattr:`technique_name` so that
    ...
    def find_peaks(
        ...
        starts = [
            int(max(0, min(sorted_left[k], n - 2)))
            for k in range(len(sorted_indices))
        ]
        ends = [
            int(max(starts[k] + 1, min(sorted_right[k], n - 1)))
            for k in range(len(sorted_indices))
        ]
        ...
        for k in range(len(sorted_indices) - 1):
            if ends[k] >= starts[k + 1]:
                ...
                valley_idx = left_apex + int(
                    np.argmin(y[left_apex : right_apex + 1])
                )
                ends[k] = valley_idx
